...
## Transporte de baixo nível

O `ZapoProvider` expõe `sendRawMessage()` para integrações avançadas que precisam enviar um `Proto.IMessage` já montado. O Core não interpreta nem oferece builders para protocolos experimentais; esse método é intencionalmente de baixo nível.
